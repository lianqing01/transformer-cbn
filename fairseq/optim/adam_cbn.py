...
@register_optimizer('adam_cbn')
class FairseqAdam_CBN(FairseqOptimizer):
    ...
    def __init__(self, args, params, params_lag=None):
        super().__init__(args)
        if torch.cuda.is_available():
            try:
                from apex.optimizers import FusedAdam as _FusedAdam  # noqa
                self._optimizer = FusedAdam(params, params_lag, **self.optimizer_config)
            except ImportError:
                self._optimizer = Adam(params, params_lag, **self.optimizer_config)
        else:
            self._optimizer = Adam(params, params_lag, **self.optimizer_config)
    ...
    def optimizer_config(self):
        ...
        return {
            'lr': self.args.lr[0],
            'lr_cbn': self.args.lr_cbn[0],
            'weight_decay_cbn': self.args.weight_decay_cbn,
...
class Adam(torch.optim.Optimizer):
    ...
    def __init__(self, params, params_lag, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
...
class FusedAdam(torch.optim.Optimizer):
    ...
    def __init__(self, params,
        ...
        super(FusedAdam, self).__init__([{'params': params},
                                    {'params': params_lag, 'lr':lr_cbn,
                                     'weight_decay': weight_decay_cbn}],
                                     defaults)
